...
#ifdef ESP32
#include "../ESP32AsyncTCP/AsyncTCP.h"
...
class AsyncMqttClient {
 public:
  AsyncMqttClient(IAsyncClient& client);
  ~AsyncMqttClient();
 ...
 private:
  ...
  // TCP
  void _onConnect(IAsyncClient* client);
  void _onDisconnect(IAsyncClient* client);
  static void _onError(IAsyncClient* client, int8_t error);
  void _onTimeout(IAsyncClient* client, uint32_t time);
  static void _onAck(IAsyncClient* client, size_t len, uint32_t time);
  void _onData(IAsyncClient* client, char* data, size_t len);
  void _onPoll(IAsyncClient* client);
