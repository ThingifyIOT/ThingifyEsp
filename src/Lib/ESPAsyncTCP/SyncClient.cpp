...
SyncClient::SyncClient(IAsyncClient *client, size_t txBufLen)
  : _client(client)
...
int SyncClient::connect(IPAddress ip, uint16_t port){
  ...
  _client->onConnect([](void *obj, IAsyncClient *c){ ((SyncClient*)(obj))->_onConnect(c); }, this);
...
void SyncClient::_onConnect(IAsyncClient *c){
  _client = c;
  if(_tx_buffer != NULL){
    cbuf *b = _tx_buffer;
    _tx_buffer = NULL;
...
void SyncClient::_attachCallbacks_AfterConnected(){
  _client->onAck([](void *obj, IAsyncClient* c, size_t len, uint32_t time){ ((SyncClient*)(obj))->_sendBuffer(); }, this);
  _client->onData([](void *obj, IAsyncClient* c, void *data, size_t len){ ((SyncClient*)(obj))->_onData(data, len); }, this);
  _client->onTimeout([](void *obj, IAsyncClient* c, uint32_t time){ c->close(); }, this);
}

void SyncClient::_attachCallbacks_Disconnect(){
  _client->onDisconnect([](void *obj, IAsyncClient* c){ ((SyncClient*)(obj))->_onDisconnect(); delete c; }, this);
}
